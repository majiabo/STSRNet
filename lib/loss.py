...
class BlurDegreeLoss(nn.Module):
    def __init__(self):
        super(BlurDegreeLoss, self).__init__()
        self.l1_loss = nn.L1Loss()

    def forward(self, xs, ys):
        losses = []
        for x, y in zip(xs, ys):
            losses.append(self.l1_loss(x, y))
...
class WeightedL1(nn.Module):
    ...
    def forward(self, x, y, blur_map):
        v = self.l1_loss(x*(blur_map*self.a+self.b), y*(blur_map*self.a+self.b))
        return v
...
class PerceptionLoss(nn.Module):
    ...
    31：第五次maxpool后
    """
    def __init__(self, floor, img_num=5):
    ...
    def forward(self, out_images, target_images):
        ...
        for i in range(self.img_num):
            perception_loss = self.mse_loss(self.loss_network(out_images[:, i*3:(i+1)*3, ...]),
                                            self.loss_network(target_images[:, i*3:(i+1)*3, ...]))
            temp.append(perception_loss)
...
if __name__ == '__main__':
    loss_fn = WeightedL1(1)
    x = torch.randn((12, 3, 128, 128))
    y = torch.randn((12, 3, 128, 128))
    weight = torch.randn((12, 3, 128, 128))
    v = loss_fn(x, y, weight)
    print()
