...
sys.path.append('../')
from attention_strategy.ablation_ws.configs import Ablation5by5
from attention_strategy.ablation_ws.net import STTRNetAblationStudy
...
G = STTRNetAblationStudy(sr=True, out_num=len(args.out_img), top_k=args.top_k, ws=args.window_size).cuda()
G.train()
...
for epoch in range(args.start_epoch, args.stop_epoch):
    ...
    with torch.no_grad():
        ...
        for index, (lr_img, hr_img, blur_maps) in enumerate(test_loader):
            ...
            psnrs += psnr
            ssims += ssim
        Writer.add_scalar('scalar/test_psnr', sum(psnrs)/len(psnrs), epoch)
        Writer.add_scalar('scalar/test_ssim', sum(ssims)/len(ssims), epoch)

    gg_path = os.path.join(args.checkpoints_path, 'G_{}.pth'.format(epoch))
    if device_num > 1:
